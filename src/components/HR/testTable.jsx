...
export default function TestTable({ email, name, onSubmit, goBack }) {
    ...
    const checkExistingAppraisal = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          setOverallValue(data.overallValue)
    ...
    <div className="border rounded-lg w-full p-10 bg-white">
      {isSubmitted && (
        <div className="mt-4 text-red-500 font-bold">
          You have already submitted the assessment form for this employee.
        </div>
      )}
      <div className="relative w-full overflow-auto">
        <h3 style={{ fontSize: '24pt', fontWeight: '600' }}>Employee Assessment Form</h3>
        <h6>{"For: " + name}</h6>
        <h6>{"Email: " + email}</h6>
        <Table>
          <TableHeader>
            ...
            {attributes.map((attribute, index) => (
              <TableRow key={index}>
                ...
                <TableCell>
                  <Textarea type="text"
                    className="w-full"
                    placeholder="Enter you improvemnt plan here."
                    onChange={(e) => { handleImprovementPlanChange(index, e.target.value) }}
                    value={improvementPlans[index]} disabled={(selectedValues[index] == 5)} />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
