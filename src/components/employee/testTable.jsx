...
export default function TestTable({ email }) {
  ...
  const [improvementPlans, setImprovementPlans] = useState(
    attributes.map(() => "No goals set for this quarter") // Initialize with empty strings for each attribute
  );
  ...
  useEffect(() => {
    const checkExistingAppraisal = async () => {
      ...
      try {
        ...
        const snapshot2 = await get(GoalsRef);
        if (snapshot2.exists()) {
          let goals1  = snapshot2.val()[Object.keys(snapshot2.val())[0]]
          let im_arr = new Array()
          goals1.attributes.forEach(att => {
            im_arr[im_arr.length] = att.goal;
          })
          setImprovementPlans(im_arr)
        }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-[40%]">Your Target</TableHead>
